  ...
  const processEngagementTrends = (results) => {
    ...
    results.forEach((post) => {
      if (post.published_on) {
        ...
        if (!engagementByDate[date]) {
          engagementByDate[date] = { likes: 0, comments: 0, retweets: 0, count: 0 };
        }
        engagementByDate[date].likes += post.like_count || 0;
        engagementByDate[date].comments += post.comment_count || 0;
        engagementByDate[date].retweets += post.retweet_count || 0;
    ...
    return Object.keys(engagementByDate)
      .map((date) => ({
        ...
        retweets: Math.round(engagementByDate[date].retweets / engagementByDate[date].count),
      }))
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={dashboardData.engagementTrends}>
                      ...
                      <Line type="monotone" dataKey="retweets" stroke="#F59E0B" name="Reposts" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
